...
df=pd.read_csv('D:\learning\DAT210x-master\Module4\Datasets\kidney_disease.csv')
ef=df.dropna()
cd=df.drop(['id', 'classification'], axis=1)
ab=cd.dropna()
ab.pcv = pd.to_numeric(df.pcv, errors='coerce')
ab.wc = pd.to_numeric(df.wc, errors='coerce')
ab.rc = pd.to_numeric(df.rc, errors='coerce')
ab=pd.get_dummies(ab,['rbc','pc','pcc','ba','htn','dm','vad','appet','pe','ane'])
ab
# Create some color coded labels; the actual label feature
# will be removed prior to executing PCA, since it's unsupervised.
# You're only labeling by color so you can see the effects of PCA
labels = ['red' if i=='ckd' else 'green' for i in ef.classification]
labels
...
if scaleFeatures: ab = helper.scaleFeatures(ab)
...
ax = helper.drawVectors(T, pca.components_, ab.columns.values, plt, scaleFeatures)
T = pd.DataFrame(T)
